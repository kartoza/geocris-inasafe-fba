DROP MATERIALIZED VIEW IF EXISTS public.mv_flood_event_population_sub_district_summary;
CREATE MATERIALIZED VIEW public.mv_flood_event_population_sub_district_summary AS
    WITH non_flooded_count as (
        ...
        where a.district_id is not null and a.sub_district_id is not null and a.village_id is not null
        group by a.flood_event_id, a.district_id, a.sub_district_id
        )
    select
    ...
    from
         flooded_count
         LEFT JOIN sub_district_trigger_status trigger_status on flooded_count.flood_event_id = trigger_status.flood_event_id and flooded_count.sub_district_id = trigger_status.sub_district_id
         JOIN non_flooded_count on flooded_count.sub_district_id = non_flooded_count.sub_district_id
